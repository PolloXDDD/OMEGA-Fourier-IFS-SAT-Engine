...
This section gives an executable implementation of the concrete
Fourier--IFS self-oracular architecture.  The program accepts DIMACS CNF
files directly in Google Colab and writes a text report containing the
decision, a satisfying assignment when one is found, an independent model
verification, and statistics for the finite quotient explored during the
computation.

The implementation uses the canonical residual CNF as an exact executable
quotient state.  Therefore, whenever two recursive paths produce the same
residual formula, the paths are merged and the previously computed result is
reused.  Clause-local degree-one Walsh coefficients are used as a spectral
branch-ordering signal.

The implementation is exact whenever it returns \texttt{SAT} or
\texttt{UNSAT}.  Its executable quotient provides a direct experimental
realization of the state-merging mechanism developed above.  A general
polynomial upper bound on the number of distinct quotient states is a
separate mathematical complexity claim and is not inferred merely from the
program's execution.

\subsection{One-Cell Colab Program}

\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize,breaklines=true,columns=fullflexible]
...
\texttt{name\_omega\_result.txt}.  For satisfiable formulas the report
contains a DIMACS-compatible model line
\[
    \texttt{v }\ell_1\ \ell_2\ \cdots\ \ell_n\ \texttt{0},
\]
together with a direct re-evaluation of every input clause.  For
unsatisfiable formulas it returns \texttt{UNSAT}.  If the optional execution
limit is reached, the output is \texttt{UNKNOWN\_TIMEOUT} rather than an
incorrect SAT/UNSAT claim.

The reported quotient statistics include the number of canonical residual
states, memoization collisions, branch points, unit reductions, recursion
depth, and total recursive calls.
